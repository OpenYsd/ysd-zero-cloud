...
> = {
  projects: {
    title: 'Projects',
    eyebrow: 'Workspace',
    description: 'Applications, environments, and connected repositories.',
  },
  deployments: {
    title: 'Deployments',
    eyebrow: 'Delivery',
    description: 'Plans analysed and recorded against the cost guard.',
  },
  databases: {
    title: 'Databases',
    eyebrow: 'Data',
    description: 'Your Cloudflare D1 database, with Studio and SQL access.',
  },
  storage: {
    title: 'Storage',
    eyebrow: 'Data',
  ...
  'game-servers': {
    title: 'Game Servers',
    eyebrow: 'Compute',
    description: 'Create and monitor community game infrastructure.',
  },
  nodes: {
    title: 'Nodes',
    eyebrow: 'Compute',
    description: 'Bring your own machine and share safe spare capacity.',
  },
  logs: {
    title: 'Logs',
    eyebrow: 'Observability',
    description: 'Every action this workspace has taken.',
  },
  networking: {
    title: 'Networking',
    eyebrow: 'Edge',
    description: 'Domains, routes, tunnels, and traffic health.',
  },
  secrets: {
    title: 'Secrets',
    eyebrow: 'Security',
  ...
  usage: {
    title: 'Usage',
    eyebrow: 'Limits',
    description: 'Measured free-tier consumption and projected cost.',
  },
  shield: {
    title: 'YSD Shield',
  ...
  admin: {
    ...
    description:
      'Roles, suspension, and sign-in state for every account on this instance.',
  },
  settings: {
    title: 'Settings',
    eyebrow: 'Workspace',
    description: 'Workspace defaults and integration configuration.',
  },
};
    ...
    <>
      <article className="cloud-card p-5">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="mt-1 text-[10px] text-white/28">
                Cloudflare D1 · binding DB · Global
              </p>
            </div>
          </div>
          <Badge
        ...
        <div className="mt-5 grid grid-cols-2 gap-4 border-t border-white/[0.06] pt-4 text-xs sm:grid-cols-4">
          <SmallMetric
            label="Size"
            value={bytes === null ? 'not reported' : formatBytes(bytes)}
          />
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <NavLink
          ...
          className="cloud-card group flex items-center gap-4 p-5"
        >
          <span className="icon-well icon-well-violet">
            <Table2 />
          </span>
          <span>
            ...
            <span className="mt-1 block text-[11px] text-white/30">
              Browse live tables with credentials masked.
            </span>
          </span>
        ...
        >
          ...
          <span>
            ...
            <span className="mt-1 block text-[11px] text-white/30">
              Run guarded statements against D1.
            </span>
          </span>
          <ArrowUpRight className="ml-auto size-4 text-white/18 group-hover:text-white/55" />
        </NavLink>
      ...
      <section className="cloud-card overflow-hidden">
        <div className="border-b border-white/[0.065] px-5 py-4">
          ...
          <p className="mt-1 text-[10px] text-white/27">
            Read live from the database schema.
          </p>
        </div>
        <Table className="text-[11px]">
          <TableHeader>
              ...
              >
                ...
                <TableCell className="px-4 py-3">
                  {table.hasPrimaryKey ? (
                    ...
                    <Badge
                      variant="outline"
                      className="border-amber-400/20 text-amber-300"
                    >
                      missing
                    </Badge>
                  )}
    ...
    <div className="grid gap-4 lg:grid-cols-[1.2fr_.8fr]">
      <article className="cloud-card p-5">
        <div className="flex justify-between">
          <div>
            ...
            <p className="mt-1 text-[10px] text-white/27">
              Measured {relativeTime(measuredAt, now)}
            </p>
          </div>
          <Badge
            variant="outline"
            ...
            <div key={reading.id}>
              <div className="mb-2 flex justify-between text-[10px]">
                <span className="text-white/42">
                  {reading.label}{' '}
                  <span className="text-white/22">· {reading.provider}</span>
                </span>
                <span
                  className={
                    reading.measured
                      ? 'font-mono text-white/28'
                      : 'font-mono text-white/18'
                  }
                >
                  {formatUsage(reading)}
                </span>
      ...
      <article className="cloud-card p-5">
        ...
        <div className="space-y-3 text-[11px]">
          {[...new Set(readings.map((reading) => reading.provider))].map(
            (provider) => (
        ...
        <p className="mt-6 text-[10px] leading-4 text-white/25">
          This workspace has no billing relationship with any provider. There is
...
const PREVIEW_COPY: Partial<
  Record<
    Section,
    { note: string; metrics: Parameters<typeof MetricGrid>[0]['items'] }
  >
> = {
  ai: {
...
export function PreviewSection({ section }: { section: Section }) {
  ...
  if (!preview)
    return (
      <PreviewNotice>
        This surface is not connected to live data yet.
      </PreviewNotice>
    );
