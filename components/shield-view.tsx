...
import { CheckCircle2, Loader2, RefreshCcw, ShieldAlert, ShieldCheck, TriangleAlert } from 'lucide-react';
      ...
      {open.length === 0 ? (
        ...
        <section className="cloud-card overflow-hidden">
          <div className="border-b border-white/[0.065] px-5 py-4">
            ...
            <p className="mt-1 text-[10px] text-white/27">Each row includes the change that clears it.</p>
          </div>
          <Table className="text-[11px]">
            <TableHeader>
              <TableRow className="border-white/[0.06] hover:bg-transparent">
                {['Finding', 'Resource', 'Severity', 'How to fix', 'First seen'].map((column) => (
            ...
            <TableBody>
              {open.map((finding) => (
                <TableRow key={finding.id} className="border-white/[0.05] hover:bg-white/[0.02]">
                  ...
                  <TableCell className="px-4 py-3 text-white/42">{relativeTime(finding.firstSeenAt, data.now)}</TableCell>
                </TableRow>
              ))}
            </TableBody>
          </Table>
