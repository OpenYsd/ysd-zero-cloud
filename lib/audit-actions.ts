...
export const EVIDENCE_ACTIONS = [
  ...
  },
  {
    ...
    metadataKeys: ['nodeId', 'deployments', 'reasonCode'],
    route: 'lib/server/nodes.ts',
    critical: true,
    why: 'Declaring a node permanently lost kills its credential for good and is the only door to moving its deployments.',
  },
  {
    action: 'deployment.ownership_transfer',
    resourceType: 'deployment',
    metadataKeys: ['sourceNodeId', 'nodeId', 'artifactId', 'desiredRevision'],
    route: 'lib/server/nodes.ts',
    critical: true,
    why: 'Moving a deployment between nodes is the one act that changes who may run it, so who moved it and from where must survive.',
  },
  {
    action: 'deployment.artifact_import',
    resourceType: 'deployment',
    metadataKeys: ['artifactId', 'sourceArtifactId', 'sourceNodeId', 'nodeId', 'desiredRevision'],
    route: 'lib/server/nodes.ts',
    critical: true,
    why: 'An imported artifact is the only artifact whose bytes were never built here, so its provenance is recorded rather than inferred.',
  },
  {
