...
void test('a binding marks its integration as bound', () => {
  const catalog = getIntegrationCatalog({ DB: {} });
...
void test('an integration needs every key before it counts as configured', () => {
  ...
  assert.equal(partial.find((entry) => entry.id === 'cloudflare')?.status, 'mock');

  const complete = getIntegrationCatalog({
    CLOUDFLARE_API_TOKEN: 'token',
    CLOUDFLARE_ACCOUNT_ID: 'account',
  });
  assert.equal(complete.find((entry) => entry.id === 'cloudflare')?.status, 'configured');
});
...
void test('every integration is declared free-tier only', () => {
  for (const entry of getIntegrationCatalog({})) {
    assert.equal(entry.freeTierOnly, true, `${entry.id} must be free-tier only`);
  }
});

void test('capability helpers agree with the catalog', () => {
  assert.equal(hasGithubOAuth({ GITHUB_CLIENT_ID: 'id', GITHUB_CLIENT_SECRET: 'secret' }), true);
  assert.equal(hasGithubOAuth({ GITHUB_CLIENT_ID: 'id' }), false);
  assert.equal(hasCloudflareApi({ CLOUDFLARE_API_TOKEN: 't', CLOUDFLARE_ACCOUNT_ID: 'a' }), true);
...
void test('repository references are parsed from the shapes an operator types', () => {
  ...
  assert.deepEqual(parseRepository('  OpenYsd/ysd-zero-cloud  '), expected);
  assert.deepEqual(parseRepository('https://github.com/OpenYsd/ysd-zero-cloud'), expected);
  assert.deepEqual(parseRepository('https://github.com/OpenYsd/ysd-zero-cloud.git'), expected);
  // A pasted deep link keeps the owner and repo and drops the rest.
  assert.deepEqual(parseRepository('https://github.com/OpenYsd/ysd-zero-cloud/tree/main'), expected);
});
